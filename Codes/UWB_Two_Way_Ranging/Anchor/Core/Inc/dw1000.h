...
// Sub-Register Addresses (Crucial for RF initialization)
...
#define DW1000_TX_FCTRL_TXBR_110K 0x00
#define DW1000_TX_FCTRL_TXBR_850K 0x01
#define DW1000_TX_FCTRL_TXBR_6M8  0x02
...
/* Delayed ("scheduled") transmission — this is the missing piece that makes
 * real DS-TWR possible. It lets a node know its OWN future TX timestamp
 * BEFORE the frame goes out, so that timestamp can be embedded inside the
 * frame itself (instead of being guessed / deferred to a later cycle).
 * Mirrors DW1000Class::setDelay() + the TXDLYS path in thotro/arduino-dw1000.
 */
...
/* 40-bit timestamp wraparound mask. SYS_TIME/RX_TIME/TX_TIME are 40-bit
 * hardware counters (5 bytes) that roll over roughly every 17.2s. Any
 * subtraction between two timestamps MUST be masked with this so a wrap
 * mid-cycle doesn't blow up into a huge 64-bit underflow. */
#define DW1000_TS_MASK ((dw1000_timestamp_t)0xFFFFFFFFFFULL)
