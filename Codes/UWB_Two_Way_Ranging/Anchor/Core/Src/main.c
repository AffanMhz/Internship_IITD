...
/**
  ******************************************************************************
  * @file           : main_ANCHOR.c
  * @brief          : DW1000 DS-TWR ANCHOR — compact serial output
  *
  * Serial output format (Arduino Serial Plotter compatible):
  * Every successful range:  "D:<meters>\n"
  * Stats (every 10 cycles): "S:ok=<n>,prej=<n>,fto=<n>,err=<n>\n"
  * No signal heartbeat:     "H:listening\n"
  * Fatal errors only (rare): "E:<code>\n"
  *
  * Timing-critical path is UART-free:
  * T2 capture  → no UART before RESPONSE TX setup
  * T6 capture  → no UART before FINAL frame read
  ******************************************************************************
  */
...
/* ===================================================================== *
 * CALIBRATION OFFSET
 * Calculate as: (Measured Distance) - (Actual Physical Distance)
 * If you see ~155.15m at exactly 1.0m actual distance, offset = 154.15.
 * Adjust this slightly to fine-tune your specific hardware's antenna delay.
 * ===================================================================== */
#define CALIBRATION_OFFSET_M  154.35
...
static uint8_t uart_tx_dma_buf[128];

/* Diagnostic counters — printed on stats line every STATS_INTERVAL cycles */
#define STATS_INTERVAL  10
static uint32_t cnt_ok    = 0;
static uint32_t cnt_prej  = 0;  /* preamble rejected */
static uint32_t cnt_fto   = 0;  /* FINAL frame timeout */
static uint32_t cnt_err   = 0;  /* other errors (LDE, SFD timeout, etc.) */
static uint32_t cnt_cycle = 0;
...
int main(void)
{
  ...
  /* One-time boot message — identify this node and output format */
  UART_Print("ANCHOR DS-TWR ready. Format: D:<m> S:ok,prej,fto,err H:listening\r\n");
  ...
  while (1)
  {
    ...
    while (!rx_success)
    {
      ...
      if (!DW1000_WaitForIrqStatus(50, sys_status)) {
        /* Heartbeat: only print when genuinely idle, not inside an exchange */
        if (HAL_GetTick() - last_heartbeat > 2000) {
          UART_Print("H:listening\r\n");
      ...
      if (rxdfr && rxfcg) {
        /* Capture T2 immediately — before any other SPI access */
        t_poll_rx = dw1000_GetRxTimestamp(&dw1000);
        last_heartbeat = HAL_GetTick();  /* suppress heartbeat mid-exchange */
        rx_success = 1;
      }
    }
    ...
    if (rx_buf[0] != FC_POLL) { cnt_err++; continue; }

    /* ---- 2. Send RESPONSE (capture T3) — NO UART in this block --------- */
    DW1000_Force_TRXOFF(&dw1000);
    DW1000_ClearAllStatus_Manual(&dw1000);
    ...
    if (DW1000_WaitForIrqStatus(100, sys_status)) {
      if ((sys_status[0] >> 7) & 1) { tx_sent = 1; }
    }

    if (!tx_sent) {
      cnt_err++;
      UART_Print("E:resp_tx_timeout\r\n");
      continue;
    }

    t_resp_tx = dw1000_GetTxTimestamp(&dw1000);  /* T3 */
    dw1000_timestamp_t reply1 = (t_resp_tx - t_poll_rx) & DW1000_TS_MASK;

    /* ---- 3. Wait for FINAL (capture T6) — NO UART in this block -------- */
    DW1000_Force_TRXOFF(&dw1000);
    DW1000_ClearAllStatus_Manual(&dw1000);
    dw1000_StartReceive(&dw1000, 1);
    ...
    while (HAL_GetTick() - rx_final_start < 250) {
      if (!DW1000_WaitForIrqStatus(250 - (HAL_GetTick() - rx_final_start), sys_status)) {
        break;
      }
      if (((sys_status[1] >> 5) & 1) && ((sys_status[1] >> 6) & 1)) {
        t_final_rx = dw1000_GetRxTimestamp(&dw1000);  /* T6 — captured first */
        rx_success = 1;
        break;
      }
    }
    ...
    if (!rx_success) {
      cnt_fto++;
      cnt_err++;
      /* Single compact error token — no multi-word string */
      UART_Print("E:fto\r\n");
      DW1000_Force_TRXOFF(&dw1000);
      continue;
    }
    ...
    if (rx_buf[0] != FC_FINAL) { cnt_err++; continue; }

    /* ---- 4. DS-TWR distance computation -------------------------------- */
    dw1000_timestamp_t T1 = 0, T4 = 0, T5 = 0;
    for (int b = 0; b < 5; b++)
      T1 |= ((dw1000_timestamp_t)rx_buf[1 + b]) << (8 * b);
    for (int b = 0; b < 5; b++)
      T4 |= ((dw1000_timestamp_t)rx_buf[6 + b]) << (8 * b);
    for (int b = 0; b < 5; b++)
      T5 |= ((dw1000_timestamp_t)rx_buf[11 + b]) << (8 * b);

    dw1000_timestamp_t round1 = (T4 - T1) & DW1000_TS_MASK;
    dw1000_timestamp_t reply2 = (T5 - T4) & DW1000_TS_MASK;
    dw1000_timestamp_t round2 = (t_final_rx - t_resp_tx) & DW1000_TS_MASK;

    int64_t R1 = (int64_t)round1;
    int64_t R2 = (int64_t)round2;
    int64_t P1 = (int64_t)reply1;
    int64_t P2 = (int64_t)reply2;

    int64_t d1 = R1 - P1;
    int64_t d2 = R2 - P2;

    int64_t numerator   = (d1 * P2) + (d2 * P1) + (d1 * d2);
    int64_t denominator = R1 + R2 + P1 + P2;

    if (denominator == 0) { cnt_err++; UART_Print("E:denom0\r\n"); continue; }

    int64_t tof_raw = numerator / denominator;

    if (tof_raw < 0) { cnt_err++; continue; }
    ...
    /* ---- 5. Output — compact, plotter-friendly ------------------------- */
    cnt_ok++;
    cnt_cycle++;

    if (distance_m > 0.0 && distance_m < 1000.0) {
      int dist_int  = (int)distance_m;
      int dist_frac = (int)((distance_m - (double)dist_int) * 100);
      if (dist_frac < 0) dist_frac = -dist_frac;
      /* "D:<int>.<frac>\n" — Serial Plotter reads the number after the colon */
      snprintf(msg, sizeof(msg), "D:%d.%02d\r\n", dist_int, dist_frac);
    } else {
      /* Out-of-range: emit a sentinel value so the plotter line stays visible */
      snprintf(msg, sizeof(msg), "D:-1\r\n");
      cnt_err++;
    }
    UART_Print(msg);

    /* Stats line every STATS_INTERVAL successful measurements */
    if (cnt_cycle % STATS_INTERVAL == 0) {
      snprintf(msg, sizeof(msg), "S:ok=%lu,prej=%lu,fto=%lu,err=%lu\r\n",
               cnt_ok, cnt_prej, cnt_fto, cnt_err);
      UART_Print(msg);
    }
  }
}
