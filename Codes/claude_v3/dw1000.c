...
uint8_t dw1000_ReceiveDataFrameReady(dw1000_HandleTypeDef *dw1000) {
  uint8_t sys_status[DW1000_SYS_STATUS_LEN];
  dw1000_ReadData(dw1000, DW1000_SYS_STATUS, sys_status, DW1000_SYS_STATUS_LEN);
  return dw1000_IsSet(sys_status, DW1000_SYS_STATUS_RXFCG); // RXFCG = good CRC; RXDFR alone means data ready but CRC unknown
}
...
uint8_t dw1000_WaitForCPLOCK(dw1000_HandleTypeDef *dw1000, uint32_t timeout_ms) {
  /*
   * Wait for the clock PLL to lock (CPLOCK, SYS_STATUS bit 1).
   *
   * Per DWM1000 datasheet Table 1 the crystal takes ~3 ms to start;
   * 200 ms is a generous timeout that covers worst-case startup.
   *
   * The DW1000 SPI max frequency is 3 MHz until CPLOCK is asserted
   * (datasheet Table 2). The caller should reinitialise the SPI
   * peripheral to a faster speed after this function returns 1.
   */
  ...
  while ((HAL_GetTick() - start) < timeout_ms) {
    dw1000_ReadData(dw1000, DW1000_SYS_STATUS, sys_status, DW1000_SYS_STATUS_LEN);
    if (dw1000_IsSet(sys_status, DW1000_SYS_STATUS_CPLOCK)) {
      return 1;
    }
  }
  return 0;
