...
/* Wait (with ms timeout) for CPLOCK bit in SYS_STATUS.
 * Returns 1 if PLL locked within timeout_ms, 0 if timeout expired.
 * MUST be called after Init and before any TX/RX.
 * Per DWM1000 datasheet Table 2: SPI max 3 MHz before CPLOCK. */
