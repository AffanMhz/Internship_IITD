...
/**
  ******************************************************************************
  * @file           : main_TAG.c
  * @brief          : DW1000 DS-TWR TAG — fixed for sawtooth pattern
  *
  * Changes from uploaded tag_node.c:
  *
  *   [D] t_resp_rx (T4) moved to immediately after rxdfr&&rxfcg confirm,
  *       before GetDataReceivedLength/GetDataReceived. RX_TIME is latched
  *       by hardware and unaffected by those reads, but this is the
  *       defensively correct order.
  *
  *   [DS-TWR] TAG now sends TWO intervals in the FINAL payload:
  *       bytes [1..5]  = round1 = T4-T1  (current cycle)
  *       bytes [6..10] = reply2 = T5-T4  (PREVIOUS cycle)
  *     T5 cannot be known before FINAL is transmitted, so reply2 is
  *     captured after TX and stored for the next cycle. First cycle
  *     sends reply2=0 (anchor skips calculation that cycle).
  *
  *   [G] Dead variables (t_final_tx, prev_t_round, t_round_32) removed.
  *       Misleading UART print "Next t_round=0:0" also removed.
  ******************************************************************************
  */
...
int main(void)
{
   ...
   *   T1 = t_poll_tx   TAG transmits POLL
   *   T4 = t_resp_rx   TAG receives RESPONSE
   *   T5 = t_final_tx  TAG transmits FINAL
   * round1 = T4-T1 is sent in FINAL bytes [1..5].
   * reply2 = T5-T4 (previous cycle) is sent in FINAL bytes [6..10]. */
  dw1000_timestamp_t t_poll_tx  = 0;
  dw1000_timestamp_t t_resp_rx  = 0;
  dw1000_timestamp_t t_final_tx = 0;
  dw1000_timestamp_t reply2_prev = 0;  /* T5-T4 from previous cycle */
  ...
  while (1)
  {
    ...
    UART_Print("[TAG] << POLL sent (T1). Waiting for RESPONSE...\r\n");
    ...
    while (HAL_GetTick() - rx_start < 300) {
      ...
      if (rxdfr && rxfcg) {
        /* [FIX D] Capture T4 immediately — before GetDataReceived.
         * RX_TIME is hardware-latched at LDEDONE and independent of
         * buffer reads, but this ordering is defensively correct. */
    ...
    if (rx_buf[0] != FC_RESPONSE) {
      UART_Print("[TAG] Frame received but not FC_RESPONSE. Dropping.\r\n");
      HAL_Delay(200);
      continue;
    }

    dw1000_timestamp_t round1 = t_resp_rx - t_poll_tx;  /* T4-T1 */

    UART_Print("[TAG] >> RESPONSE received (T4). Sending FINAL...\r\n");

    /* ---- 3. Send FINAL (T5) ----------------------------- */
    /* FINAL payload:
     *   bytes [1..5]  = round1 = T4-T1  (this cycle)
     *   bytes [6..10] = reply2 = T5-T4  (PREVIOUS cycle; 0 on first cycle) */
    DW1000_Force_TRXOFF(&dw1000);
    DW1000_ClearAllStatus_Manual(&dw1000);
    ...
    for (int b = 0; b < 5; b++)
      tx_buf[6 + b] = (uint8_t)(reply2_prev >> (8 * b));
    ...
    /* Capture T5 now — TXFRS is set, hardware has latched TX_TIME */
    t_final_tx  = dw1000_GetTxTimestamp(&dw1000);
    reply2_prev = t_final_tx - t_resp_rx;  /* T5-T4, used next cycle */

    sprintf(msg, "[TAG] FINAL sent (T5). round1=%lu reply2_next=%lu\r\n",
            (uint32_t)(round1 & 0xFFFFFFFF),
            (uint32_t)(reply2_prev & 0xFFFFFFFF));
    UART_Print(msg);
