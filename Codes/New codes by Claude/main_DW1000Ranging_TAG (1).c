...
int main(void)
{
   ...
   * User Manual for reliable RX operation - must match ANCHOR exactly
   * (channel 5, 16 MHz PRF, 110 kbps, 1024-symbol preamble). */
  ...
  sprintf(msg, "[TAG] AGC_TUNE1=0x%04X (exp 0x8870) LDE_CFG1=0x%02X (exp 0x6D) DRX_TUNE2=0x%08lX (exp 0x311A002D)\r\n",
  ...
  sprintf(msg, "[TAG] PANID=0x%04X SHORTADDR=0x%04X CHAN tx=%u rx=%u prf=%u txpre=%u rxpre=%u\r\n",
  ...
  while (1)
  {
    ...
    if (!sent)
    {
      ...
      sprintf(msg, "[TAG] POLL TX timeout (TXFRS never set). SYS_STATUS=%02X%02X%02X%02X%02X SYS_STATE=0x%06lX\r\n",
    ...
    if (!frame_received)
    {
      ...
      sprintf(msg, "[TAG] Timeout waiting for RESPONSE. SYS_STATUS=%02X%02X%02X%02X%02X SYS_STATE=0x%06lX "
    ...
    {
      sprintf(msg, "[TAG] RX frame ready but length invalid: %u\r\n", len);
      UART_Print(msg);
      dw1000_ClearReceiveStatus(&dw1000);
      HAL_Delay(500);
      continue;
    ...
    sprintf(msg, "[TAG] RX frame: len=%u fc=0x%02X\r\n", len, rx_buf[0]);
    UART_Print(msg);

    if (rx_buf[0] != FC_RESPONSE)
    {
      UART_Print("[TAG] Unexpected frame type, expected RESPONSE\r\n");
      HAL_Delay(500);
      continue;
    }
    ...
    if (!sent)
    {
      UART_Print("[TAG] FINAL TX timeout (TXFRS never set)\r\n");
      HAL_Delay(500);
      continue;
    }
    ...
    sprintf(msg, "[TAG] FINAL sent. t_final_tx=%llu t_round(this cycle)=%llu (%.4f us, sent next cycle)\r\n",
            (unsigned long long)t_final_tx, (unsigned long long)t_round,
