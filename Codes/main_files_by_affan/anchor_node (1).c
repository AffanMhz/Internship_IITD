...
/**
  ******************************************************************************
  * @file           : main_ANCHOR.c
  * @brief          : DW1000 Diagnostic Two-Way Ranging - ANCHOR
  ******************************************************************************
  */
...
int main(void)
{
  ...
  /* 1. Master Init (Handles default PAC, PLEN, and Power perfectly) */
  ...
  UART_Print("\r\n========================================\r\n");
  UART_Print("[ANCHOR] Initialized without Saturation. Awaiting POLL...\r\n");
  UART_Print("========================================\r\n");
  ...
  while (1)
  {
    ...
    while (!rx_success)
    {
      ...
      uint8_t rxprd  = (sys_status[1] >> 0) & 1; // Preamble Detected
      uint8_t rxdfr  = (sys_status[1] >> 5) & 1; // Data frame ready
      uint8_t rxfcg  = (sys_status[1] >> 6) & 1; // FCS Good
      uint8_t rxfce  = (sys_status[1] >> 7) & 1; // FCS Error
      uint8_t rxphe  = (sys_status[1] >> 4) & 1; // PHY Header Error
      uint8_t rxrfsl = (sys_status[2] >> 0) & 1; // Sync Loss
      uint8_t ldeerr = (sys_status[2] >> 2) & 1; // LDE Error
      uint8_t rxsfdto= (sys_status[3] >> 2) & 1; // SFD Timeout
      uint8_t rxprej = (sys_status[4] >> 1) & 1; // Receiver Preamble Rejection
      ...
      /* LIVE DIAGNOSTICS: See exactly when the signal hits the antenna */
      if (rxprd && !preamble_notified) {
         UART_Print("[ANCHOR] -> Signal Preamble Detected! Waiting for SFD...\r\n");
         preamble_notified = 1;
      }

      if (rxprej) {
         UART_Print("[ANCHOR] -> ERROR: Preamble Rejected (RXPREJ). Antennas too close or PAC mismatch!\r\n");
         DW1000_Force_TRXOFF(&dw1000);
         DW1000_ClearAllStatus_Manual(&dw1000);
         dw1000_StartReceive(&dw1000, 1);
         preamble_notified = 0;
      ...
      if (rxfce || rxphe || rxrfsl || rxsfdto || ldeerr) {
        if (ldeerr) UART_Print("[ANCHOR] -> ERROR: LDE Algorithm crashed.\r\n");
        if (rxsfdto) UART_Print("[ANCHOR] -> ERROR: SFD Timeout (Preamble found, but payload missing).\r\n");
        if (rxphe) UART_Print("[ANCHOR] -> ERROR: PHY Header corrupted.\r\n");
    ...
    UART_Print("[ANCHOR] >> RX POLL Success! Sending RESPONSE...\r\n");
    ...
    UART_Print("[ANCHOR] >> RX FINAL Success!\r\n");

    /* ---- 4. Compute distance ----------------------------- */
    /* ---- 4. Compute distance ----------------------------- */
    t_round = 0; // Read the Tag's T_round from the FINAL payload
    for (int b = 0; b < 5; b++) {
      t_round |= ((dw1000_timestamp_t)rx_buf[1 + b]) << (8 * b);
    }

    // Calculate ToF right now using the current cycle's t_reply_this
    if (t_round > t_reply_this) {
        dw1000_timestamp_t tof_raw = (t_round - t_reply_this) / 2;
        double distance_m = DW1000_Time_TimestampToMeters(tof_raw);
        distance_m -= CALIBRATION_OFFSET_M;   // measured at a known 1.00 m reference

        int dist_int = (int)distance_m;
        int dist_frac = (int)((distance_m - dist_int) * 1000);

        if (distance_m > 0.0 && distance_m < 1000.0) {
            sprintf(msg, "[ANCHOR] *** SUCCESS! Distance: %d.%03d m ***\r\n", dist_int, dist_frac);
        } else {
            sprintf(msg, "[ANCHOR] Distance OOB: %d.%03d m\r\n", dist_int, dist_frac);
        }
        UART_Print(msg);
    } else {
        UART_Print("[ANCHOR] Timing error (t_round <= t_reply_this)\r\n");
    }

    // Note: t_reply_prev and have_prev_reply variables can be deleted from main()
